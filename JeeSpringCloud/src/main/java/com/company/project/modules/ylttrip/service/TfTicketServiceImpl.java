...
public class TfTicketServiceImpl extends AbstractBaseService<TfTicketDao, TfTicket> implements ITfTicketService{
	...
	public TfTicket getCache(String id) {
		//获取缓存数据
		TfTicket tfTicket=(TfTicket)redisUtils.get(RedisUtils.getIdKey(TfTicketServiceImpl.class.getName(),id));
		if( tfTicket!=null) {
			return tfTicket;
		}
		//获取数据库数据
		...
		//设置缓存数据
		redisUtils.set(RedisUtils.getIdKey(TfTicketServiceImpl.class.getName(),id),tfTicket);
		return tfTicket;
	}
	...
	public List<TfTicket> totalCache(TfTicket tfTicket) {
		//获取缓存数据
		String totalKey = RedisUtils.getTotalKey(TfTicketServiceImpl.class.getName(),JSON.toJSONString(tfTicket));
		List<TfTicket> tfTicketList=(List<TfTicket>)redisUtils.get(totalKey);
		if(tfTicketList!=null) {
			return tfTicketList;
		}
		//获取数据库数据
	...
	public List<TfTicket> findListCache(TfTicket tfTicket) {
		//获取缓存数据
		String findListKey = RedisUtils.getFindListKey(TfTicketServiceImpl.class.getName(),JSON.toJSONString(tfTicket));
	...
	public TfTicket findListFirstCache(TfTicket tfTicket) {
		//获取缓存数据
		String findListFirstKey = RedisUtils.getFindListFirstKey(TfTicketServiceImpl.class.getName(),JSON.toJSONString(tfTicket));
		TfTicket tfTicketRedis=(TfTicket)redisUtils.get(findListFirstKey);
		if(tfTicketRedis!=null) {
	...
	public Page<TfTicket> findPageCache(Page<TfTicket> page, TfTicket tfTicket) {
		//获取缓存数据
		String findPageKey =  RedisUtils.getFindPageKey(TfTicketServiceImpl.class.getName(),JSON.toJSONString(page)+JSON.toJSONString(tfTicket));
	...
	public void save(TfTicket tfTicket) {
		...
		super.save(tfTicket);
		//设置清除缓存数据
		redisUtils.remove(RedisUtils.getIdKey(TfTicketServiceImpl.class.getName(),tfTicket.getId()));
		//清除列表和页面缓存数据
		redisUtils.removePattern(RedisUtils.getFindListKeyPattern(TfTicketServiceImpl.class.getName()));
		redisUtils.removePattern(RedisUtils.getFinPageKeyPattern(TfTicketServiceImpl.class.getName()));
	}
